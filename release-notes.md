...
社区预览版：官方站点切到 `grok.3rcd.com`，去掉直充余额轨，贴边把手按火绒节奏用设备像素画 2px 刻度。

## 下载

- `3R.Waterline_0.1.3_x64-setup.exe`：Windows 安装版（推荐）。
- `three_r_waterline.exe`：Windows 便携版。
- `3R.Waterline_0.1.3_universal.dmg`：macOS 通用安装镜像（Apple Silicon 与 Intel）。

## 已包含

- 登录与额度读取改为 `https://grok.3rcd.com/`，不再请求 `https://ai.3rcd.com/`。
...
- 四向贴边隐藏：14px 把手、周绿/月蓝各 10 点、点高 2px、点距 2px、组距 8px。刻度按设备像素整数绘制，避免 125%/150% 缩放下高低不一。

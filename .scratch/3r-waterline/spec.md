...
32. As a privacy-conscious user, I want the application to access only the official authentication flow and subscriptions page, so that API keys, orders, payments, account balances, and unrelated personal data remain outside its reach.
...
- Normalize a subscriptions page into a collection of Subscriptions. A Supported Subscription requires valid weekly and monthly Used Amounts, limits, and reset intervals. The amount shown in the overlay is Remaining Amount, calculated as limit minus Used Amount; each value is rejected if parsing or arithmetic is invalid.
...
- Locally advance Reset Countdown values only at the day-and-hour precision supplied by the page, then recalibrate them after every successful subscriptions-page read.
...
- Enable Edge Hide by default. It activates only when a user drags the overlay to a work-area edge of the current display, leaves an eight-pixel content-free tab, expands on hover or click, and never activates solely from inactivity. Persist account-independent overlay dimensions and placement preferences per display when feasible; recover to the primary display if a saved display is absent.
...
- Accessing 3R API key, order, purchase, recharge, payment, profile, or non-subscription pages.

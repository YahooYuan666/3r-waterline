...
32. As a privacy-conscious user, I want the application to access only the official authentication flow and subscriptions page, so that API keys, orders, payments, account profile details, and unrelated personal data remain outside its reach.
...
- **Direct-balance removal:** Do not read `/auth/me`, available-balance fields, or any synthetic `Grok 直充余额` display item. The overlay shows only Supported Subscription weekly/monthly quota.
- **2026-08-25 visual refresh:** Keep the two display modes and every current interaction unchanged, but replace the pale dual-chamber treatment with the approved `Graphite Rail` visual system: charcoal translucent base, white tabular figures, emerald weekly fill, cobalt monthly fill, low-contrast consumed areas, compact square corners, and a restrained single-border treatment. In Traffic Monitor, regular subscription views retain exactly two progress rails. This is a visual-only re-layout of existing controls, settings, and edge tab behavior.
...
- Accessing 3R API key, order, purchase, recharge, payment, profile, account-balance, or non-subscription pages.

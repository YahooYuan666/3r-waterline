...
- 网络不可达、服务端 5xx 或无效响应不再清除本机 Login State，也不再被前端误判为 `authentication-required`。
- 已验证额度在后续刷新失败时继续保留，并显示“暂时无法连接 3R，保留上次额度”；初次读取失败显示“暂时无法连接 3R”。
- 悬浮窗提供“重新连接”按钮；只有明确收到 401/403 或没有会话时才进入认证状态。
- 前端新增原生错误分类与 UI 重试回归；Rust 新增认证 HTTP 状态判定回归。

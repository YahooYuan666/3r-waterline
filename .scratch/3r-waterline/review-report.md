...
## 2026-08-25 直充余额与 Graphite Rail 复核

### 结论

范围内未发现会把直充余额误显示为额度、泄漏未授权账户字段或破坏订阅读取安全失败策略的问题。

### 独立检查

- `available_balance_cny` 只接受白名单余额字段和有限、非负数；测试证明 `email` 等账户字段不会被取用。
- `/auth/me` 的 401 会沿用认证失效路径；其他余额读取失败回退为隐藏余额项，因此已验证周/月数据不被该附加信息拖垮。
- 前端以 `availableBalance` 的结构分支渲染直充余额，明确不创建 `PeriodQuota`，因此不存在分母、水位高度和 reset 文本；App 测试覆盖该状态。
- 本地 240 × 116 横条回归截图检查：周/月订阅恰为两条轨道；`Grok 直充余额` 为一条独立金额轨。水瓶状态无填充水位，满足不伪造额度语义。
- 新鲜命令：前端 49 项测试、生产构建、Rust 5 项测试、Cargo check/fmt 与 diff hygiene 通过。

...
4. 在“设置”中切换“圆形水瓶”和“Traffic Monitor”，并选择大/中/小界面尺寸。关闭设置后，确认设置标题栏可拖动；Traffic Monitor 只有周、月两条自适应横条，没有滚动条，中/小档文字不加粗且不重叠。标题栏显示当前订阅名，两侧按钮可切换订阅，也可在设置中开启自动切换。确认不再出现 `Grok 直充余额` 翻页项。
5. 把悬浮窗拖到屏幕左、右、上或下边缘并松开，确认约 250ms 后仅保留清晰可见的 14px 把手；周/月各 10 个点（点高 2px、点距 2px、组距 8px）居中成簇，不铺满整条窗口，绿色/蓝色点对应剩余比例。鼠标移入把手后完整悬浮窗恢复，鼠标移开后自动重新贴边；拖动悬浮窗离开边缘后才解除贴边状态。

...
- 根据用户提供的顶部/侧边参考图，将周/月两组 10 段刻度的组内间距调整为 2px、组间间距调整为 8px；左右与上下方向相同。
- `npm test -- --run` 仍为 49 passed，`npm run build` 与 `npm run desktop:build` 均通过。
- 浏览器安全策略禁止用数据页挂载临时贴边状态预览；未绕过该策略，也未停止用户正在运行的安装版。最终原生贴边视觉留给新包人工确认。
